...
class MakeOutput(luigi.Task):
    ...
    def peanut_butter(self, db_col):
        ...
        pb_df['brand'] = ["Coles Sanitarium - Crunchy",
                          "The Forage Company - Smooth",
                          "Sanitarium - Crunchy", "Aldi Bramwells - Crunchy",
                          "Aldi Bramwells - American Style"]
        ax = sns.barplot(y="brand", x="energy_100g",
                         data=pb_df, fill="brands", orient="h")

        # Can make a nice plot
        ax.set_title('Top 5 Energy Rich Peanut Butters in Australia',
                     fontsize=12)
    ...
    def product_categories(self, db_col):
        ...
        food_regex = "(pork|chicken|tofu)"
        pc_df["food_group"] = pc_df.categories_en.str.extract(food_regex)
        ...
        summary_df = nn_df.groupby(['food_group']).agg(['mean', 'median',
                                                        'std'])
